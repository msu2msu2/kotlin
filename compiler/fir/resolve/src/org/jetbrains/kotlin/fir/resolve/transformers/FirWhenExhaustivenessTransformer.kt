...
private object WhenOnSealedClassExhaustivenessChecker : WhenExhaustivenessChecker() {
    ...
    override fun computeMissingCases(
        ...
        (allSubclasses - checkedSubclasses).mapNotNullTo(destination) {
            when (it) {
                is FirClassSymbol<*> -> WhenMissingCase.IsTypeCheckIsMissing(it.classId)
    ...
    private object ConditionChecker : AbstractConditionChecker<Flags>() {
        ...
        private fun processBranch(symbolToCheck: AbstractFirBasedSymbol<*>, isNegated: Boolean, flags: Flags) {
            val checkedSubclasses = symbolToCheck.collectAllSubclasses(flags.session)
            if (checkedSubclasses.none { it in flags.allSubclasses }) {
                return
            }
            if (isNegated) {
                if (flags.checkedSubclasses.containsAll(checkedSubclasses)) {
                    // is A && !is A -> contains all subclasses
                    flags.checkedSubclasses.addAll(flags.allSubclasses)
                    return
                }
                flags.checkedSubclasses.addAll(flags.allSubclasses)
                flags.checkedSubclasses.removeAll(checkedSubclasses)
            } else {
